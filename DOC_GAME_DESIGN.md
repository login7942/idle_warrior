...
### 2.2. 강화 및 내구도 (Enhancement & Durability) - [개편 중]
*   **[신규] 장착 슬롯 강화 (Slot Enhancement)**:
    *   **대상**: 아이템이 아닌 6개의 장착 슬롯(무기, 투구, 갑옷, 신발, 반지, 목걸이) 자체를 강화.
    *   **범위**: +0 ~ +100레벨.
    *   **특징**: 장비를 교체해도 슬롯의 강화 수치는 영구 유지됨. 실패 시 재료만 소모되며 파손이나 하락 없음.
*   **[기존] 아이템 개별 강화 (Item Enhancement)**:
    *   **최대 강화**: +20
    *   **승급 (Promotion)**: +20 달성 시 재료를 소모하여 다음 티어 +10 장비로 진화. (슬롯 강화 정착 후 폐지 예정)
    *   **내구도 실패 패널티**: 11강 이후 실패 시 내구도 감소 및 파손 가능성.
    *   **계승 시스템**: 파손 시 강화 수치의 70%가 슬롯에 저장되어 주입됨.
...
##### 1. [신규] 장착 슬롯 강화 (Slot Enhancement)
*   **UI 위치**: 가방(Inventory) 상단 장착 슬롯 영역에 통합 표시.
*   **성공 확률**: 1~20강(100%), 21~50강(80%~40%), 51~100강(30%~5% 점진적 하락)
*   **실패 패널티**: 강화 재료(골드, 강화석) 소모 외 없음. (파손/하락 없음)
*   **스탯 보너스**: 슬롯별 주 능력치(공격력/방어력/체력)에 대해 레벨당 일정 % 보너스 부여. 장비 교체 시 자동으로 새 장비에 적용됨.

##### 2. [기존] 아이템 개별 강화 (Item Enhancement - Legacy)
*   **상태**: 유지하되 향후 폐지 예정. 신규 아이템은 슬롯 강화의 영향만 받도록 점진적 전환.
*   **계승**: 기존 +20강 장비 보유 유저는 슬롯 강화 수치로 보상받는 등 마이그레이션 이벤트 고려.
