...
*   **쾌속 등반 (Jump)**: 몬스터 처치 속도에 따라 스테이지를 건너뜀 (1초 미만: 2단계, 2초 미만: 1단계).
...
*   **스냅샷 방식**: 사용자가 직접 [방치모드]를 활성화한 시점의 효율(GPM/EPM/KPM)을 60초간 측정 후 기록.
*   **보상 계산**: `획득량 = 경과 시간(분) * 스냅샷 효율 * 방치 효율(기본 30%)`
*   **한도**: 최대 5시간(300분)까지 누적 가능.
