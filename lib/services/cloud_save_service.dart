...
class CloudSaveService {
  ...
  Future<bool> saveToCloud(Map<String, dynamic> gameData) async {
    try {
      ...
      if (userId == null) {
        print('[CloudSave] 저장 실패: 사용자 미로그인');
        return false;
      }
      ...
      print('[CloudSave] 클라우드 저장 성공');
      return true;
    } catch (e) {
      print('[CloudSave] 클라우드 저장 실패: $e');
      return false;
    }
  }
  ...
  Future<Map<String, dynamic>?> loadFromCloud() async {
    try {
      final userId = _supabase.auth.currentUser?.id;
      if (userId == null) {
        print('[CloudSave] 로드 실패: 사용자 미로그인');
        return null;
      }
      ...
      if (response == null) {
        print('[CloudSave] 클라우드 데이터 없음');
        return null;
      }
      
      print('[CloudSave] 클라우드 로드 성공 (버전: ${response['version']})');
      return {
  ...
  Future<bool> deleteCloudSave() async {
    try {
      ...
      print('[CloudSave] 클라우드 데이터 삭제 완료');
      return true;
    } catch (e) {
      print('[CloudSave] 클라우드 삭제 실패: $e');
      return false;
    }
  }
}
