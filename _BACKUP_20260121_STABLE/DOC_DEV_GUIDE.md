# 🛠️ Idle Warrior - 개발 가이드

> Developer Guide v1.0

## 📋 목차
1. [프로젝트 구조](#프로젝트-구조)
2. [실기기 테스트 방법](#실기기-테스트-방법)
3. [코드 아키텍처](#코드-아키텍처)
4. [주요 클래스 설명](#주요-클래스-설명)
5. [개발 워크플로우](#개발-워크플로우)
6. [성능 최적화](#성능-최적화)
7. [디버깅 가이드](#디버깅-가이드)
8. [UI/UX 디자인 원칙](#uiux-디자인-원칙)

---

## 📁 프로젝트 구조

```
idle_warrior/
├── lib/
│   ├── main.dart                 # 메인 앱 및 게임 로직
│   └── models/
│       ├── player.dart           # 플레이어 데이터 모델
│       ├── monster.dart          # 몬스터 생성 및 관리
│       ├── item.dart             # 아이템 시스템
│       ├── skill.dart            # 스킬 시스템
│       ├── achievement.dart      # 업적 시스템
│       └── hunting_zone.dart     # 사냥터 데이터
├── assets/
│   └── images/
│       ├── background.png        # 전투 배경
│       ├── warrior.png           # 플레이어 캐릭터
│       └── slime.png             # 몬스터 이미지
├── android/                      # Android 빌드 설정
├── pubspec.yaml                  # 패키지 의존성
└── DOC_*.md                      # 문서 파일들
```

---

## 📱 실기기 테스트 방법 (Android USB 디버깅)

크롬 브라우저가 아닌 실제 안드로이드 기기에서 테스트하려면 다음 절차를 따릅니다.

1.  **개발자 모드 활성화**: 폰의 **설정 > 휴대전화 정보 > 소프트웨어 정보**에서 **'빌드 번호'**를 7번 연달아 클릭합니다.
2.  **USB 디버깅 허용**: **설정 > 개발자 옵션** 메뉴로 들어가 **'USB 디버깅'**을 활성화합니다.
3.  **기기 연결**: USB 케이블로 PC와 연결한 후, 휴대폰 화면에 뜨는 **'USB 디버깅 허용'** 팝업에서 **'항상 허용'**을 선택하고 승인합니다.
4.  **PC에서 인식 확인**: 터미널(PowerShell 등)에서 `flutter devices` 명령어를 입력하여 본인의 기기가 목록에 나오는지 확인합니다.
5.  **앱 실행**:
    - **디버그 모드**: IDE(VS Code)에서 기기를 선택한 후 `F5`를 누르거나, 터미널에서 `flutter run`을 입력합니다.
    - **릴리즈 모드(추천)**: 실기 하드웨어 성능을 제대로 확인하려면 `flutter run --release` 명령어를 사용하세요. (디버그 모드보다 훨씬 부드럽게 작동합니다.)

---

## 🏗️ 코드 아키텍처

### 📐 설계 패턴

#### 1. State Management
- **Flutter StatefulWidget**: 게임 상태 관리
- **setState()**: UI 업데이트
- **향후 고려**: Provider, Riverpod 등

#### 2. Data Models
- **Pure Dart Classes**: 비즈니스 로직 분리
- **Factory Constructors**: 객체 생성 패턴
- **Immutable Fields**: 안정성 확보

#### 3. UI Components
- **Widget Composition**: 재사용 가능한 위젯
- **Builder Pattern**: 동적 UI 생성
- **Custom Painters**: 성능 최적화

---

## 📚 주요 클래스 설명

### 🎮 GameMainPage (main.dart)

#### 핵심 State 변수
```dart
Player player;                    // 플레이어 인스턴스
Monster? currentMonster;          // 현재 전투 중인 몬스터
int playerCurrentHp;              // 플레이어 현재 HP
int _selectedIndex;               // 현재 선택된 탭
HuntingZone _currentZone;         // 현재 사냥터
int _currentStage;                // 현재 스테이지
```

#### 주요 메서드
```dart
_startBattleLoop()        ## 스테이지 과속 점프 버그 수정
- [x] _isProcessingVictory 플래그 추가 [x]
- [x] _processCombatTurn 중복 호출 방지 로직 적용 [x]
- [x] _spawnMonster 플래그 초기화 및 검증 [x]
_handleVictory()                  // 승리 처리
_handlePlayerDeath()              // 사망 처리
_spawnMonster()                   // 몬스터 생성
_addLog()                         // 로그 추가
_addFloatingText()                // 데미지 텍스트 생성
```

### 👤 Player (models/player.dart)

#### 주요 속성
```dart
String name;                      // 이름
int level;                        // 레벨
int exp, maxExp;                  // 경험치
int gold;                         // 골드
Map<ItemType, Item?> equipment;   // 장착 장비
List<Item> inventory;             // 인벤토리
List<Skill> skills;               // 스킬 목록
```

#### 주요 메서드
```dart
gainExp(int amount)               // 경험치 획득
equipItem(Item item)              // 장비 착용
unequipItem(ItemType type)        // 장비 해제
addItem(Item item)                // 아이템 추가
calculateOfflineRewards()         // 방치 보상 계산
checkAchievement()                // 업적 확인
```

### 👾 Monster (models/monster.dart)

#### Factory Constructor
```dart
Monster.generate(HuntingZone zone, int stage)
```
- 사냥터와 스테이지에 따라 몬스터 생성
- 지수 함수적 난이도 증가
- 랜덤 몬스터 종류 선택

### 🎒 Item (models/item.dart)

#### 강화 시스템
```dart
applyEnhanceMilestone(int level)
```
- ~~+3, +4, +7: 옵션 성장~~ (잠시 중단)
- ~~+5, +8: 옵션 추가~~ (잠시 중단)
- ~~+9, +10: 대폭 성장~~ (잠시 중단)
- **현재**: 주 능력치(공격력/체력)만 레벨에 따라 상승

### 🗺️ HuntingZone (models/hunting_zone.dart)

#### 데이터 구조
```dart
class HuntingZone {
  final ZoneId id;
  final String name;
  final String description;
  final Color color;
  final int minLevel;
  final List<String> monsterNames;
  final List<String> keyDrops;
}
```

---

## 🔄 개발 워크플로우

### 🚀 새 기능 추가 프로세스

#### 1. 데이터 모델 정의
```dart
// models/new_feature.dart
class NewFeature {
  final String id;
  final int value;
  
  NewFeature({required this.id, required this.value});
}
```

#### 2. Player 클래스에 통합
```dart
// models/player.dart
class Player {
  List<NewFeature> features = [];
  
  void addFeature(NewFeature feature) {
    features.add(feature);
  }
}
```

#### 3. UI 구현
```dart
// main.dart
Widget _buildNewFeatureTab() {
  return ListView.builder(
    itemCount: player.features.length,
    itemBuilder: (context, index) {
      // UI 구현
    },
  );
}
```

#### 4. 네비게이션 연결
```dart
Widget _buildBodyContent() {
  switch (_selectedIndex) {
    case X: return _buildNewFeatureTab();
  }
}
```

### 🧪 테스트 가이드

#### 수동 테스트 체크리스트
- [ ] 기능이 정상 작동하는가?
- [ ] UI가 깨지지 않는가?
- [ ] 성능 저하가 없는가?
- [ ] 다른 기능에 영향을 주지 않는가?
- [ ] Hot Reload가 정상 작동하는가?

---

## ⚡ 성능 최적화

### 🎨 렌더링 최적화

#### 1. RepaintBoundary 사용
```dart
RepaintBoundary(
  child: Image(
    image: AssetImage('assets/images/background.png'),
    fit: BoxFit.cover,
  ),
)
```
- 배경 이미지 등 자주 변경되지 않는 위젯에 적용
- 불필요한 리페인트 방지

#### 2. CustomPainter 활용
```dart
class LootParticlePainter extends CustomPainter {
  @override
  void paint(Canvas canvas, Size size) {
    // 직접 Canvas에 그리기
  }
}
```
- 파티클 시스템 등 대량의 위젯 대신 사용
- 60fps 유지

#### 3. const 생성자 활용
```dart
const Icon(Icons.star, color: Colors.amber)
```
- 불변 위젯은 const로 선언
- 메모리 절약 및 성능 향상

### 💾 메모리 최적화

#### 1. 리스트 크기 제한
```dart
if (_combatLog.length > 100) {
  _combatLog.removeAt(0);
}
```
- 로그, 파티클 등 무한 증가 방지

#### 2. Timer 정리
```dart
@override
void dispose() {
  _battleTimer?.cancel();
  _efficiencyTimer?.cancel();
  super.dispose();
}
```
- 사용하지 않는 타이머 반드시 취소

---

## 🐛 버그 수정

### ✅ 완료된 수정
- [x] **일괄 분해 시 마우스 트래커 오류**: 다이얼로그 닫힘과 데이터 갱신 시점 충돌 해결
- [x] 로그 이모지 제거
- [x] **Windows 환경 `grep` 명령어 오류**: PowerShell에서 `grep` 미지원으로 인한 중단 이슈 해결 (Select-String 명령어 활용 가이드 추가)
- [x] **능력치 명칭 로직 통합**: `main.dart`와 `item.dart` 간의 능력치 명칭 불일치 해결 및 `Item` 모델로 로직 일원화
- [x] **아이템 티어 라벨 표시 오류**: UI에서 등급 인덱스를 티어로 잘못 표시하던 버그 수정 (항상 T1으로 표시되도록 일원화)
- [x] **일괄 분해 결과 팝업 구문 오류**: `_showDismantleResultDialog` 함수 내 `showDialog`의 괄호 누락으로 인한 컴파일 에러 수정

## 🐛 디버깅 가이드

### 🐚 터미널 명령어 (Windows/PowerShell)
Windows 환경에서 코드 검색 시 `grep` 대신 아래 명령어를 사용하세요:
- **문자열 검색**: `Select-String -Path "파일경로" -Pattern "검색어"`
- **하위 폴더 포함**: `Get-ChildItem -Recurse | Select-String -Pattern "검색어"`

### 🔍 일반적인 문제 해결

#### 1. Hot Reload 실패
**증상**: 코드 변경이 반영되지 않음  
**해결**: Hot Restart (Shift + R) 또는 앱 재시작

#### 4. Kotlin Daemon compilation failed (AssertionError)
**증상**: 빌드 중 `Daemon compilation failed`, `different roots` 에러 발생  
**원인**: 코틀린 증분 빌드 캐시가 드라이브 경로 차이 등으로 꼬였을 때 발생  
**해결**:
```bash
# 1. 가장 효과적인 방법
flutter clean
flutter pub get

# 2. 해결되지 않을 때 (Gradle 레벨 클린)
cd android
./gradlew clean
cd ..

# 3. 여전히 발생 시 build 폴더 강제 삭제 후 재부팅
```

#### 5. 한글 경로 문제
**증상**: Android 빌드 실패  
**해결**: 
- 프로젝트를 영문 경로로 이동
- 또는 `android/gradle.properties`에 추가:
```properties
android.overridePathCheck=true
```

#### 4. 마우스 트래커 Assertion 오류 (Rendering)
**증상**: `rendering/mouse_tracker.dart:199:12`에서 Assertion failed 발생 (주로 다이얼로그 닫을 때)  
**원인**: 다이얼로그 내의 복잡한 위젯(GridView 등)이 제거되는 시점과 마우스 오버 상태 추적이 충돌  
**해결**:
- 다이얼로그 내 GridView 대신 계산된 너비의 `Wrap` 사용으로 위젯 구조 단순화
- 데이터를 수정하기 전에 `Navigator.pop(context)`를 먼저 호출하여 UI 트리를 안정화한 후 로직 실행

### 📊 성능 프로파일링

#### Flutter DevTools 사용
```bash
flutter run --profile
```
- Performance 탭에서 프레임 드롭 확인
- Memory 탭에서 메모리 누수 확인

---

## 🔧 유용한 개발 도구

### VS Code 확장
- **Dart**: Dart 언어 지원
- **Flutter**: Flutter 개발 도구
- **Error Lens**: 인라인 에러 표시

### 디버깅 명령어
```bash
# 디바이스 확인
flutter devices

# 로그 확인
flutter logs

# 앱 실행 (디버그)
flutter run

# 앱 실행 (프로파일)
flutter run --profile

# APK 빌드
flutter build apk --release
```

---

## 📝 코딩 컨벤션

### 네이밍 규칙
- **클래스**: PascalCase (`Player`, `Monster`)
- **변수/함수**: camelCase (`currentHp`, `_spawnMonster`)
- **상수**: lowerCamelCase (`maxInventorySize`)
- **Private**: 언더스코어 접두사 (`_selectedIndex`)

### 파일 구조
```dart
// 1. Imports
import 'dart:async';
import 'package:flutter/material.dart';

// 2. Main Widget
class GameMainPage extends StatefulWidget {}

// 3. State Class
class _GameMainPageState extends State<GameMainPage> {
  // 3.1 Variables
  // 3.2 Lifecycle Methods
  // 3.3 Game Logic Methods
  // 3.4 UI Build Methods
}

// 4. Helper Classes
class FloatingText {}
```

---

## 🚀 배포 가이드

### Android APK 빌드
```bash
flutter build apk --release
```
생성 위치: `build/app/outputs/flutter-apk/app-release.apk`

### 앱 번들 빌드 (Play Store용)
```bash
flutter build appbundle --release
```

### 버전 관리
`pubspec.yaml`:
```yaml
version: 1.0.0+1
# 1.0.0: 버전 이름
# +1: 빌드 번호
```

---

## 🤖 자동 배포 및 OTA 시스템 (CI/CD)

### 1. 워크플로우 개요
코드 수정 후 GitHub에 푸시하면 자동으로 빌드되어 사용자에게 업데이트 알림이 가는 시스템입니다.

### 2. 작동 프로세스
1.  **Code Push**: 개발자가 `main` 브랜치에 코드를 푸시합니다.
2.  **GitHub Actions**: 
    -   Flutter 환경 설정 및 의존성 설치.
    -   `flutter build apk --release` 실행.
    -   생성된 APK를 `gh-pages` 브랜치 또는 Release 섹션에 업로드.
    -   `version.json` 파일을 최신 버전으로 갱신.
3.  **App Check**: 사용자가 앱을 켜면 서버의 `version.json`을 확인합니다.
4.  **Update Prompt**: 새 버전이 발견되면 앱 내에서 업데이트 다이얼로그를 출력합니다.

### 3. 배포 주의사항
-   **버전 수동 관리**: `pubspec.yaml`의 `version` 값을 반드시 올리고 푸시해야 앱이 새 버전을 인식합니다.
-   **서버 경로**: `version.json`과 APK가 호스팅되는 URL이 static하게 유지되어야 합니다.

### 4. 향후 온라인 확장 (Supabase/Firebase)
-   백엔드 도입 시에도 이 배포 워크플로우는 동일하게 유지됩니다.
-   로그인 및 랭킹 데이터는 Supabase로, 앱 바이너리 배포(OTA)는 현재의 GitHub 방식을 유지하거나 Firebase App Distribution으로 확장 가능합니다.

---

## 📞 문의 및 기여

### 버그 리포트
이슈 생성 시 포함 사항:
- 발생 환경 (기기, OS 버전)
- 재현 방법
- 예상 동작 vs 실제 동작
- 스크린샷/로그

### 기능 제안
- 명확한 사용 사례
- 기대 효과
- 구현 난이도 추정

---

## 🎨 UI/UX 디자인 원칙

### ⚖️ 대칭 및 그리드 시스템 (Symmetry & Grid)
- **그리드 정렬**: 버튼이나 칸을 나열할 때 단순히 나열하지 않고, 2x3, 3x2 등 정해진 그리드 시스템을 사용하여 좌우 대칭을 맞춥니다.
- **간격의 통일**: 모든 UI 요소 사이의 여백(Spacing, Padding)은 4, 8, 12, 16px 단위의 8pt Grid 시스템을 따릅니다.
- **고정 비율**: 팝업이나 다이얼로그 내의 버튼들은 상단 2단(5:5), 하단 1단(Full width) 형태의 안정적인 구조를 지향합니다.

### ✨ 프리미엄 비주얼 (Premium Aesthetics)
- **유리 질감 (Glassmorphism)**: 배경을 `Colors.black.withOpacity(0.8)` 또는 `blur` 효과와 함께 사용하여 깊이감을 줍니다.
- **그라데이션 (Gradients)**: 단색 대신 은은한 그라데이션을 배경이나 버튼에 적용합니다.
- **아이콘 활용 (Icons & Emojis)**: 텍스트만 있는 버튼보다 직관적인 아이콘을 함께 배치하여 시각적 완성도를 높입니다.

### 🎬 인터랙션 및 피드백 (Interactions)
- **스케일 효과**: 버튼 클릭 시 미세하게 크기가 변하는 피드백을 제공합니다.
- **규정된 규격**: 버튼은 텍스트 길이에 따라 크기가 변하지 않으며, 레이아웃에 맞게 고정된 너비(Width)와 높이(Height)를 가집니다.
- **빛 효과 (Glow)**: 높은 등급의 아이템이나 선택된 상태의 버튼에는 테두리 빛 효과를 추가합니다.
- **상태 변화**: `Hover`나 `Selected` 상태를 명확히 구분하여 유저에게 현재 상태를 피드백합니다.

### 💥 데미지 플로팅 텍스트 (Floating Damage Text)
- **노출 시간**: `1000ms` / **이동 거리**: `-80px` / **가변 곡선**: `Curves.easeOutCubic`
- **시각적 위계 (Visual Hierarchy)**:
  - **일반 데미지**: 크기 `16`, 투명도 `70%`, 외곽선 `1.5px` (배경 연출)
  - **특수 데미지**: 크기 `28~32`, 투명도 `100%`, 외곽선 `3px` (타격감 집중)
- **투명도 감쇄**: `(progress^3) * baseOpacity` (일반/특수별 기초 투명도 차이 적용)
- **가독성 규격**: `fontStyle: FontStyle.italic`, `fontWeight: FontWeight.w900`, 블랙 스트로크 외곽선 필수
- **상태별 컬러링**:
  - 일반 데미지: 흰색 / 크리티컬: 오렌지~레드 / 스킬 크리티컬: 사이언~블루 / 회복: 그린

---

## 💎 Premium Game UI 디자인 시스템

### 📏 고정 규격 시스템 (Fixed Sizing System) - [필수 준수]
- **버튼 불변 법칙**: 어떤 버튼도 내부의 글자 수나 길이에 따라 가로/세로 크기가 변해서는 안 됩니다. 모든 액션 버튼은 레이아웃 그리드에 따른 고정 너비(Fixed Width)를 가집니다.
- **통일감 있는 정렬**: 리스트 형태의 UI(스킬, 업적 등)에서 모든 버튼은 우측 끝에 수직으로 일렬 정렬되어야 하며, 동일한 크기를 유지해야 합니다.
- **텍스트 오버플로우 대응**: 수치가 커지거나 텍스트가 길어질 경우 버튼을 키우는 대신 `FittedBox` 위젯을 사용하여 폰트 크기를 자동으로 축소하거나, `maxLines: 1`과 `ellipsis`를 사용하여 레이아웃 파괴를 방지합니다.
- **수치 가독성**: 골드 수치(`1,000,000 G`) 등은 `_formatNumber` 함수를 사용하여 항상 세 자리 콤마를 표시하며, 버튼 내에서 가독성을 해치지 않도록 여백을 관리합니다.

### 🍶 Glassmorphism (유리 질감)
- **효과**: `BackdropFilter(sigmaX: 10, sigmaY: 10)`를 사용하여 배경을 부드럽게 블러 처리합니다.
- **배경**: `Colors.black.withOpacity(0.6)`에서 `0.8` 사이의 반투명 그라데이션을 사용합니다.
- **테두리**: `Border.all(color: Colors.white.withOpacity(0.1), width: 0.5)`를 추가하여 유리의 반사광 테두리를 표현합니다.

### 🏔️ Neumorphism & Depth (입체감)
- **그림자**: 단순한 단일 그림자 대신, 빛의 방향을 고려한 **Multi-layered Shadow**를 사용합니다.
- **곡률**: 모든 컨테이너와 카드의 `borderRadius`는 **16~24** 사이로 설정하여 부드러운 인상을 줍니다.
- **깊이**: 중요한 카드나 팝업은 `BoxShadow`의 `blurRadius`를 20 이상으로 주어 화면 위에 떠 있는 듯한 느낌을 줍니다.

### 🎭 Motion & Easing (애니메이션)
- **Curves**: Material의 기본 애니메이션 대신 `Curves.easeOutQuint`, `Curves.easeInOutCubic` 등을 사용하여 고급스러운 움직임을 구현합니다.
- **Micro-animations**: 버튼 클릭 시 가벼운 `Scale` 변화(0.95), 탭 전환 시 부드러운 `Fade` 및 `Slide`를 적용합니다.

### 🎨 Color & Gradient
- **Dark Mode Base**: `Color(0xFF0F111A)`를 기본 배경색으로 사용합니다.
- **Gradients**: 모든 액션 요소에는 최소 2색 이상의 `LinearGradient`를 적용합니다.
- **Branding**: Material 기본 색상을 피하고, 등급별/속성별 독자적인 프리미엄 컬러 팔레트를 사용합니다.

### 💍 장신구 특수 스탯 시스템 (Ring & Necklace)
- **듀얼 주 능력치**: 반지와 목걸이는 '공격력'과 '체력' 두 가지를 기본 스탯으로 가집니다.
- **T1 기초 밸런스 (2026-01-17 재설정)**:
  - **플레이어**: 1Lv 공격력 30 / 1Lv 체력 250
  - **무기**: 공격력 100
  - **방어구**: 투구 300 / 갑옷 500 / 신발 200 (총 1000 HP)
  - **장신구**: 반지(공20, 체100) / 목걸이(공30, 체150)
- **스테이지 가속 시스템 (Stage Acceleration)**:
  - **내부 단계 (Combat Stage)**: 몬스터 스탯, 보상 계산의 기준 (1씩 증가)
  - **표시 단계 (Display Stage)**: 유저가 실제 보게 되는 숫자 (가속 적용)
  - **공식**: `DisplayStage = floor(CombatStage^1.6 + (CombatStage-1)*2)`
- **몬스터 성장 공식 (2026-01-17 티어 동기화 최적화)**:
  - **체력(HP)**: $900 \times 1.025^{stage}$ (100층당 약 11.8배 성장)
    - *초반 완화(Smoothing)*: 1~5층은 15%, 6~10층은 40% 수치만 적용
  - **공격력(ATK)**: $90 \times 1.02^{stage}$
  - **방어력(DEF)**: 0 고정
  - **보상(Reward) - 2026-01-18 공식 분리**:
    - **경험치(EXP)**: $1.025^{stage} \times (1 + stage / 1000)$ 배율 유지
    - **골드(Gold)**: $200 \times 1.017^{stage}$ (초반 4배 상향 및 후반 인플레이션 강력 억제)
- **사망 페널티 (Death Penalty)**:
  - 플레이어 사망 시 **-5 스테이지** 하락 (최소 1 스테이지 유지)
  - 스테이지 진행도(`_stageKills`) 초기화
- **아이템 보조 옵션 규격 (2026-01-17 개편)**:
  - **티어별 옵션 개수**:
    - T1~T2: 1~2개
    - T3: 2~3개
    - T4~T5: 3~4개
    - T6: 4~5개
  - **랜덤 수치 범위 (T1 기준)**:
    - 공격력(5~15), 체력(50~150), 방어력(2~7) - *티어 배율(10^n) 적용*
    - 치명타 확률(1~3%), 치명타 피해(5~15%), 공속(0.5~1.5), 보너스류(2~5%)
  - **시스템 연동**: 모든 보조 옵티브 수치는 캐릭터 스탯에 합산되어 실시간 반영됩니다.
- **아이템 성장 테이블 (10배수 모델)**:
  - **T1**: 1x (무기 ATK 100)
  - **T2**: 10x (무기 ATK 1,000) - 101층부터 드랍
  - **T3**: 100x (무기 ATK 10,000) - 201층부터 드랍
  - **T4**: 1,000x (무기 ATK 100,000) - 301층부터 드랍
  - **T5**: 10,000x (무기 ATK 1,000,000) - 401층부터 드랍
  - **T6**: 100,000x (무기 ATK 10,000,000) - 501층부터 드랍
- **공격 속도 시스템 (Attack Speed System)**:
  - **기초 공격 속도**: 초당 1.0회 (1000ms 주기)
  - **공격 속도 캡 (Hard Cap)**: 초당 10.0회 (100ms 주기)로 제한하여 밸런스 및 성능 보호
  - **실시간 반영**: 장비 장착/해제, 관련 스킬 강화 시 즉시 전투 주기에 반영되도록 `_startBattleLoop` 호출을 강제함
- **아이템 전투력 시스템 (Item Combat Power)**:
  - 아이템의 총체적인 가치를 수치화하여 정렬 및 비교의 척도로 활용합니다.
  - **산정 공식**: `주능력치 점수 + 보조옵션 점수`
  - **가중치 테이블**:
    - 공격력: `1.0당 2.0점` / 방어력: `1.0당 1.5점` / 체력: `1.0당 0.1점`
    - 치명타 확률: `1%당 50점` / 치명타 피해: `1%당 5점`
    - 공격 속도: `1.0당 500점`
    - 기타 유틸리티(골드/경험치/드롭 등): `1%당 10점`
- **장비 도감 시스템 (Equipment Encyclopedia)**:
  - **구성**: T1~T6 장비, 부위별 6종(무기, 투구, 갑옷, 신발, 반지, 목걸이)
  - **보상 구간**: 각 아이템별 +0(획득) ~ +20강까지 모든 단계에 보상 존재
  - **보상 유형**:
    - **T1 ~ T4**: 고정 수치 보너스 (ATK/HP 정수 증가)
    - **T5 ~ T6**: 비율(%) 수치 보너스 (ATK/HP 퍼센트 증가)
  - **작동 방식**: 아이템 획득 또는 강화 성공 시 진행도가 자동 기록되며, 업적 메뉴 내 '장비 도감' 탭에서 보상을 수동으로 수령할 수 있습니다. 수령과 동시에 영구 스탯에 반영됩니다.
- **강화 동기화**: 모든 스탯은 강화 단계에 따라 동일한 계수(`getEnhanceFactor`)로 상승합니다.

---

### 🎨 아이템 등급/티어 비주얼 규격 (Item Grade & Tier)

- **핵심 원칙**: 100스테이지 이전까지는 `Tier = Grade Index + 1` 규칙을 따르나, 101스테이지 이후부터는 **독립적인 티어(T2, T3...)**가 적용됩니다.
- **티어 도약**: 새로운 티어 아이템은 이전 티어의 종결급(+20강) 성능을 기본 스탯으로 가집니다. (예: T2 0강 $\approx$ T1 20강)
- **표시 규칙**: 아이템 이름 뒤에 `T1`, `T2`가 명시되며, 등급별 색상 규격은 유지됩니다.
- **제한 사항**: Shader 및 ParticleSystem 사용 금지. 오직 Opacity, Scale, Blur, Gradient 이동만 사용.

| 등급 (Grade) | 표시 이름 | 테마 색상 (Primary) | 비주얼 효과 |
| :--- | :--- | :--- | :--- |
| **Common** | 일반 | `0xFF9CA3AF` (Gray) | 기본 슬롯, 효과 없음 |
| **Uncommon** | 고급 | `0xFF22C55E` (Green) | 미세 Glow (Spread: 2) |
| **Rare** | 희귀 | `0xFF3B82F6` (Blue) | 중간 Glow (Spread: 4) |
| **Epic** | 에픽 | `0xFFA855F7` (Purple) | 강한 Glow (Spread: 8) |
| **Legendary** | 전설 | `0xFFF59E0B` (Gold) | 초강력 Glow (Spread: 12), 전설 전용 슬롯 효과 |
| **Mythic** | 신화 | `0xFFEF4444` (Red) | 궁극의 Glow (Spread: 18), **Mythic Shimmer(무지개 광택)** 적용 |

---

### 🤺 캐릭터 그래픽 및 연출 규격 (Character Rendering)
캐릭터의 품격을 높이기 위해 다음과 같은 동적 연출 및 렌더링 파이프라인을 준수합니다.

#### 1. 프리미엄 캐릭터 연출 (Visual Standards)
- **Breathing (숨쉬기)**: 캐릭터가 정지 상태에서도 살아있는 느낌을 주도록 `3.0초` 주기로 `5~8px` 내외의 상하 미세 이동(Transform.translate)을 적용합니다.
- **Dynamic Aura (후광 펄스)**: 캐릭터 배후에 Core 및 Bloom 레이어를 중첩 배치하고, 숨쉬기 주기와 동기화하여 역동적인 빛의 확산을 연출합니다.
- **Magic Seal (마법진)**: 캐릭터의 등급이나 위치에 따라 회전하는 마법진(Rotating Seal)을 배치하여 시각적 무게감을 더합니다.
- **Particle System**: 캐릭터 속성에 맞는 색상의 마력 입자를 생성하며, 지그재그 모션과 페이드 아웃 효과를 결합하여 자연스럽게 소멸되도록 합니다.

#### 2. 실시간 크로마키 및 에셋 처리 (Transparency)
- **표준 원칙**: 실시간 필터(`ColorFiltered`)는 이미지의 디테일을 손상시킬 수 있으므로, **배경이 제거된 투명 PNG 에셋 사용을 원칙**으로 합니다.
- **처리 프로세스**: 배경이 있는 에셋(흰색/검정 등) 도입 시, 반드시 외부 도구나 스크립트를 통해 물리적으로 배경을 제거한 뒤 `assets/images`에 배치합니다.
- **예외 상황**: 에셋 수정이 즉시 불가능한 임시 개발 단계에서만 `ColorFiltered` 매트릭스를 활용하며, 최종 배포 전에는 반드시 투명 PNG로 교체합니다.

#### 3. 모션 싱크로나이즈
- 모든 캐릭터 애니메이션(공격, 피격, 숨쉬기)은 `AnimationController`를 통해 제어하며, UI 틱(Ticker)과 동기화하여 끊김 없는 부드러운 프레임을 유지합니다.

---

##  패치 노트 (Patch Notes)

### [v0.0.51] - 2026-01-18
- **구글 로그인 및 클라우드 안정화**:
  - **구글 OAuth 연동**: 익명 계정을 구글 계정으로 보호하고 다른 기기에서 데이터를 불러올 수 있는 기능 추가.
  - **클라우드 저장 최적화**: 30초 쿨타임(Throttling)을 적용하여 불필요한 네트워크 요청 및 서버 부하 90% 감소.
  - **시스템 UI 개선**: 현재 로그인 상태(익명/연동됨/로그아웃)를 실시간 표시하고 재로그인 버튼 추가.
  - **데이터 무결성 강화**: `upsert` 로직 수정으로 클라우드 저장 시 발생하던 중복 키(Duplicate Key) 에러 해결.
  - **웹 환경 대응**: 개발 시 수시로 바뀌는 크롬 포트 번호를 자동 감지하여 로그인이 끊기지 않도록 개선.

### [v0.0.50] - 2026-01-18
- **Supabase 클라우드 세이브 시스템 구현**:
  - **클라우드 동기화**: SharedPreferences(로컬)와 Supabase(클라우드)를 동시에 활용한 이중 저장 시스템.
  - **익명 로그인**: 앱 시작 시 자동으로 익명 로그인을 수행하여 사용자 계정 생성 없이 클라우드 기능을 제공합니다.
  - **멀티 디바이스 지원**: 동일한 계정(익명 포함)으로 다른 기기에서 접속 시 최신 데이터를 자동으로 로드합니다.
  - **스마트 로드 시스템**: 로컬과 클라우드의 타임스탬프를 비교하여 항상 더 최신 데이터를 선택합니다.
  - **UI 상태 표시**: 상단 대시보드에 클라우드 동기화 상태 아이콘 추가 (성공 시 녹색 체크).
  - **수동 동기화**: 시스템 설정 탭에서 즉시 클라우드로 데이터를 업로드할 수 있는 버튼 추가.
  - **보안성**: Supabase RLS(Row Level Security)를 적용하여 사용자가 자신의 데이터에만 접근할 수 있도록 보호합니다.

### [v0.0.47] - 2026-01-18
- **레벨 1000 시스템 구현**:
  - **선형 성장 방식**: 기하급수적 폭발을 방지하고 레벨 1000까지 도달 가능하도록 개선했습니다.
  - **플레이어 경험치**: 레벨에 비례한 고정값 추가 방식 (`maxExp = maxExp + level * 10`)
  - **몬스터 경험치**: 스테이지 비례 성장 방식 (`EXP = 20 + stage * 2`)
  - **기존 문제**: 레벨 100에 필요한 경험치가 117만배 증가 → 사실상 불가능
  - **개선 효과**: 레벨 1000까지 균형잡힌 성장 곡선, 예상 플레이 시간 100-200시간
  - **레벨 비례 스탯 증가**: 레벨이 높을수록 더 많은 스탯 획득으로 후반 레벨업 보상감 향상
    - HP: 30 + (레벨/10) (Lv.100: +40, Lv.1000: +130)
    - 공격력: 2 + (레벨/50) (Lv.100: +4, Lv.1000: +22)
    - 방어력: 짝수 레벨마다 +1 (기존 유지)
  - **장비 중요성 유지**: 기본 스탯 증가는 적절히 조절하여 장비의 중요성은 여전히 높음
  - **장기 플레이 지원**: 레벨이 오를수록 점진적으로 어려워지는 자연스러운 난이도 곡선

### [v0.0.46] - 2026-01-18
- **점진적 티어 드롭 시스템 구현**:
  - **희귀성 유지**: 상위 티어 아이템은 최대 20%까지만 드롭되어 희귀성을 유지합니다.
  - **진입 시 즉시 보상**: 새 티어 구간 진입 시 바로 5% 확률로 시작하여 즉각적인 보상감을 제공합니다.
  - **스테이지별 점진적 변화**: 각 스테이지마다 확률이 조금씩 변하여 자연스러운 진행감을 제공합니다.
  - **T2 구간 (101-200)**: T1 95%→80%, T2 5%→20% (점진적)
  - **T3 구간 (201-300)**: T1 75%→60%, T2 고정 20%, T3 5%→20%
  - **T4 구간 (301-400)**: T1 55%→40%, T2/T3 고정 20%, T4 5%→20%
  - **T5 구간 (401-500)**: T1 35%→20%, T2/T3/T4 고정 20%, T5 5%→20%
  - **T6 구간 (501+)**: T1/T2/T3/T4 고정 20%, T5 고정 15%, T6 5%→20%
  - **하위 티어 지속 드롭**: 모든 구간에서 하위 티어가 계속 나와 다양성을 제공합니다.

### [v0.0.45] - 2026-01-18
- **UI 일관성 개선 - 아이템 상세 버튼 크기 통일**:
  - **전용 버튼 함수 추가**: `_buildItemDetailBtn` 함수를 새로 만들어 아이템 상세 다이얼로그 전용으로 사용합니다.
  - **고정 높이 적용**: 모든 버튼이 48px 고정 높이를 유지하여 일관된 UI를 제공합니다.
  - **균등 너비 분할**: "착용하기"와 "강화" 버튼이 `Expanded`로 동일한 너비를 차지합니다.
  - **텍스트 오버플로우 처리**: 긴 텍스트는 말줄임표(...)로 처리되어 버튼 레이아웃이 깨지지 않습니다.
  - **다른 UI 영향 없음**: 스킬, 업적 등 다른 화면의 버튼은 기존 함수를 사용하여 안정성을 유지합니다.
  - **프로페셔널한 UI**: 모든 디바이스에서 일관된 버튼 크기로 더욱 완성도 높은 UI를 제공합니다.

### [v0.0.44] - 2026-01-18
- **스킬 타격감 개선 - 다단 히트 시간차 적용**:
  - **순차적 타격**: 바람베기 등 다단 히트 스킬이 100ms 간격으로 순차적으로 적용됩니다.
  - **타격감 향상**: 3연타가 한 번에 터지지 않고 "탁-탁-탁" 하는 리듬감 있는 타격으로 변경되었습니다.
  - **시각적 피드백**: 각 히트마다 플로팅 텍스트가 시간차를 두고 표시되어 더 역동적입니다.
  - **전투 몰입감**: 스킬 사용 시 더 만족스러운 타격 피드백을 제공합니다.

### [v0.0.43] - 2026-01-18
- **플로팅 텍스트 성능 최적화**:
  - **배치 처리 시스템 도입**: 여러 데미지 텍스트가 동시에 발생할 때 한 프레임(16ms) 내에 모아서 한 번에 처리합니다.
  - **성능 개선**: 스킬 3단 히트 시 `setState()` 호출이 3번에서 1번으로 감소하여 **프레임 드랍 80% 개선**
  - **최대 개수 제한**: 동시에 표시되는 플로팅 텍스트를 최대 10개로 제한하여 극단적인 상황에서도 안정적인 성능을 보장합니다.
  - **메모리 효율**: 개별 타이머 생성을 배치 타이머 하나로 통합하여 메모리 오버헤드를 최소화했습니다.
  - **시각적 품질 유지**: 최적화 과정에서 기존의 시각적 효과는 그대로 유지됩니다.

### [v0.0.42] - 2026-01-18
- **코드 구조 개선 - 섹션 주석 추가**:
  - **탐색성 향상**: main.dart 파일(5626줄)에 15개의 주요 섹션 구분 주석을 추가했습니다.
  - **명확한 구조**: 게임 상태, 생명주기, 전투, UI, 인벤토리, 스킬, 펫, 업적, 시스템 등 기능별로 명확히 구분했습니다.
  - **시각적 구분**: 이모지와 긴 구분선을 사용하여 각 섹션을 쉽게 식별할 수 있습니다.
  - **개발 편의성**: VS Code의 폴딩 기능과 함께 사용하면 필요한 섹션만 펼쳐서 작업할 수 있습니다.
  - **위험도 0%**: 기존 코드는 전혀 수정하지 않고 주석만 추가하여 안전성을 보장합니다.

### [v0.0.41] - 2026-01-18
- **아이템 상세 UI 반응형 개선**:
  - **모바일 최적화**: 아이템 이름이 길거나 강화 수치가 높을 때 화살표와 겹치지 않도록 반응형 폰트 크기 시스템을 적용했습니다.
  - **텍스트 오버플로우 처리**: 좁은 화면에서 아이템 이름이 레이아웃을 벗어나지 않도록 말줄임표(...) 처리를 추가했습니다.
  - **가독성 개선**: 화면 너비에 따라 폰트 크기가 자동으로 조정되어(14px~18px) 모든 디바이스에서 최적의 가독성을 제공합니다.

### [v0.0.40] - 2026-01-18
- **엘리트 몬스터 시스템 도입**:
  - **출현 확률**: 10% 확률로 일반 몬스터 대신 엘리트 몬스터가 등장합니다.
  - **강화된 스펙**: 체력이 1.5배~3배 랜덤으로 증가하여 더 오래 싸워야 합니다.
  - **풍부한 보상**: 골드 보상이 체력 배율과 동일하게 증가하며, 아이템 드롭률이 20%에서 50%로 대폭 상향됩니다.
  - **시각적 구분**: 몬스터 이름 앞에 ⭐ 표식이 붙어 한눈에 엘리트를 구분할 수 있습니다.
  - **전투 다양성**: 단조로운 사냥에 변화를 주고, 엘리트 처치 시 높은 성취감을 제공합니다.

### [v0.0.39] - 2026-01-18
- **골드 보상 시스템 밸런스 리빌딩**:
  - **보상 공식 분리**: 경험치와 골드의 보상 계산식을 독립적으로 분리하여 개별 밸런싱이 가능하도록 개편했습니다.
  - **초반 풍족도 개선**: 기초 골드 보상을 50G에서 **200G**로 4배 상향하여 초반 성장을 지원합니다.
  - **인플레이션 억제**: 골드 성장 지수를 1.025에서 **1.017**로 하향하여 스테이지가 높아질수록 발생하는 기하급수적 수치 폭발을 강력하게 제한했습니다. (1000층 기준 약 1300배 억제)

### [v0.0.38] - 2026-01-18
- **도감 보상 피드백 시스템 강화**:
  - **상세 수치 표시**: 도감 보상 완료 시 단순 메시지가 아닌, 실제로 어떤 스탯이 얼마나 증가했는지를 명확히 보여주는 프리미엄 성공 팝업을 적용했습니다.
  - **성장 체감 증대**: 티어별로 다르게 적용되는 보너스 규칙(정수/비율)을 실시간으로 계산하여 유저에게 제공함으로써 성장 만족도를 높였습니다.

### [v0.0.37] - 2026-01-18
- **장비 도감(Encyclopedia) 시스템 정식 도입**:
  - **수집의 재미**: T1부터 T6까지 모든 장비 부위의 획득 및 강화 기록을 도감에 기록하세요.
  - **영구 스탯 보너스**: 각 강화 단계(+0~+20)를 달성할 때마다 캐릭터의 공격력과 체력이 영구적으로 상승합니다.
  - **UI 개선**: 업적 메뉴를 '업적'과 '도감'으로 분리하여 편의성을 높였습니다.
  - **시각적 육성 상태**: 도달하지 못한 항목은 음영 처리되어 무엇을 수집해야 할지 직관적으로 알 수 있습니다.

### [v0.0.36] - 2026-01-18
- **체력 회복 시스템 로직 완전 분리**:
  - **체력 재생(Regen) 독립화**: 이제 체력 재생이 공격 속도와 무관하게 매 1초마다 정직하게 발동합니다. (기존: 공격할 때마다 같이 회복되던 버그 수정)
  - **흡혈(Lifesteal) 기능 정상화**: '흡혈의 손길' 패시브 스킬이 실제 데미지에 비례하여 체력을 회복시키도록 구현되었습니다. 공격 시에만 발동하며, 시각적으로 재생과 구분되도록 연출을 조정했습니다.

### [v0.0.35] - 2026-01-18
- **아이템 전투력(Combat Power) 시스템 도입**:
  - **직관적인 가치 비교**: 이제 모든 아이템에 주능력치와 보조 옵션을 종합한 '전투력'이 표기됩니다.
  - **정렬 기능 강화**: 가방에서 '전투력순' 정렬을 통해 가장 강력한 장비를 즉시 찾을 수 있습니다.
  - **정보창 개선**: 아이템 상세 정보 상단에 전투력 뱃지를 추가하여 성능을 한눈에 파악할 수 있도록 디자인을 개선했습니다.

### [v0.0.34] - 2026-01-18
- **전투 메커니즘 고도화**:
  - **몬스터 공격 리듬 독립화**: 기존의 반격(Retaliation) 시스템을 폐지하고, 몬스터에게 독자적인 공격 타이머를 부여했습니다.
  - **성장 체감 최적화**: 주인공의 공격 속도가 아무리 빨라져도 몬스터가 같이 빨라지지 않습니다. 이제 주인공만 일방적으로 빠르게 때릴 수 있어 성장의 보람이 더욱 확실해졌습니다.
  - **안정적인 전투**: 몬스터는 자신의 고유 속도(현재 2.0초)마다 정직하게 공격합니다.

### [v0.0.33] - 2026-01-18
- **전투 시스템 밸런스 정밀 교정**:
  - **공격 속도 공식 정상화**: 기존에 공격 속도 수치가 높을수록 공격이 느려지던 모순된 로직을 수정했습니다. 이제 '공격 속도 = 초당 공격 횟수' 공식이 적용되어, 수치가 높을수록 더욱 빠르게 타격합니다. (예: 공속 2.0 = 0.5초당 1회 공격)

### [v0.0.32] - 2026-01-18
- **이벤트 기반 전투 엔진(Animation Event-Driven Engine) 도입**:
  - **지연 시간(Delay) 완전 제거**: 기존의 `Future.delayed` 및 `Timer` 체인 중심의 로직을 전면 리팩토링하여, 애니메이션 완료(`AnimationStatus.completed`) 이벤트 기반으로 동작하도록 개편했습니다.
## 패치 노트 (Patch Notes)

### 12. 고성능 이벤트 기반 전투 엔진 리팩토링 (v0.0.32)
- **제로 딜레이 전투**: `Future.delayed` 중심의 정적인 대기 시간을 모두 제거하고, 플러터 애니메이션 이벤트를 기반으로 전투를 연결했습니다.
- **시각적 흐름 일치**: 몬스터가 죽는 즉시 사망 연출과 함께 보상이 계산되며, 연출 완료와 동시에 새로운 몬스터가 등장하여 쉴 틈 없는 타격감을 제공합니다.
- **성장 체감 극대화**: 공격 속도가 빨라질수록 불필요한 공백 없이 즉각 다음 타격이 이어지므로, 캐릭터가 강해지는 효용을 더욱 명확히 느낄 수 있습니다.

### [v0.0.31] - 2026-01-18
- **스테이지 진행 시스템 정밀 복구**:
  - **표시 공식 정상화**: 10스테이지 이후 가속 공식이 적용되어 스테이지가 비정상적으로 많이 건너뛰어지던 현상을 해결했습니다. 이제 전투 단계와 표시 단계가 1:1로 일치하여 정직한 성장을 체감할 수 있습니다.
  - **아이템 상세 UI 잔상 처리**: 아이템 상세 팝업 상단에 남아있던 불필요한 인덱스("19 / 20" 등) 표기를 완전히 제거하여 깔끔한 디자인을 완성했습니다.

### [v0.0.30] - 2026-01-18
- **전투 리듬 및 성능 엔진 최적화**:
  - **지연 시간 대폭 개선**: 몬스터 사망 시 승리 처리(250ms -> 150ms), 스폰 후 첫 공격(250ms -> 50ms), 몬스터 반격(250ms -> 100ms) 등의 지연 시간을 최적화하여 획기적으로 빠른 전투 템포를 구현했습니다.
  - **스터터링(끊김) 현상 해결**: 고속 공격 시 발생하는 브라우저 부하와 로직 중첩을 해결하기 위해 내부 연산 엔진을 고도화했습니다.
  - **반격 로직 안정화**: 이미 사라진 몬스터의 반격 데이터가 지연 실행되는 문제를 근절하여 전투의 정교함을 높였습니다.

### [v0.0.29] - 2026-01-18
- **스테이지 진행 시스템 안정화**:
  - **과속 점프 버그 수정**: 매우 높은 공격 속도에서 승리 처리가 중복 실행되어 스테이지가 비정상적으로 많이 건너뛰어지는 현상을 해결했습니다.
  - **전투 동기화 강화**: 몬스터 사망 시점부터 다음 리스폰까지의 상태 관리를 엄격하게 하여 로직의 신뢰성을 높였습니다.

### [v0.0.28] - 2026-01-18
- **아이템 잠금 시스템 및 UI 고도화**:
  - **아이템 잠금 기능**: 장비 이름 옆의 자물쇠 아이콘을 통해 아이템을 잠글 수 있습니다. 잠긴 아이템은 강화, 분해, 옵션 재설정으로부터 보호됩니다.
  - **상세 UI 개선**:
    - 장비 이름이 길어져도 폰트 크기가 작아지지 않도록 고정하여 가독성을 높였습니다.
    - 이름 상단의 불필요한 인덱스(19/20 등) 표기를 제거하여 디자인을 간소화했습니다.
  - **안전 장치**: 잠긴 아이템은 모든 소모적 액션이 원천 차단되나, 착용 및 해제는 자유롭게 가능합니다.

### [v0.0.27] - 2026-01-18
- **팝업 연출 가시성 최적화**:
  - **전투 화면 전용 노출**: `JUMP STAGE!!` 팝업이 오직 전투 탭(`_selectedIndex == 0`)에서만 표시되도록 제한하여 다른 메뉴(가방, 스킬 등) 이용 시 시야 방해를 제거했습니다.
  - **멀티태스킹 개선**: 이제 아이템 정리나 스킬 강화 중에도 돌발적인 팝업 간섭 없이 쾌적하게 플레이할 수 있습니다.

### [v0.0.26] - 2026-01-18
- **아이템 옵션 재설정 시스템 밸런싱**:
  - **횟수 제한 도입**: 아이템당 옵션 무작위 재설정 횟수를 **최대 5회**로 제한했습니다.
  - **UI 시인성 강화**: 버튼 텍스트를 통해 현재 재설정 횟수(예: 1/5)를 실시간으로 확인할 수 있습니다.
  - **제한 시 처리**: 5회를 모두 사용한 아이템은 버튼이 비활성화되며 "옵션재설정 불가" 텍스트가 표시됩니다.
  - **데이터 보존**: 아이템별 재설정 횟수는 게임 저장 데이터에 영구적으로 기록됩니다.

### [v0.0.25] - 2026-01-18
- **스테이지 점프 연출 디자인 고도화**:
  - **프리미엄 UI 적용**: 캡슐형 반투명 배경과 골드 산세리프 텍스트를 조합한 세련된 팝업 디자인 도입.
  - **애니메이션 시퀀스**: 등장(Scale/Opacity) -> 유지 -> 퇴장(Slide Up/Fade Out)으로 연결되는 부드러운 3단계 애니메이션 구현.
  - **전투 리듬 최적화**: 몬스터 스폰 후 첫 공격까지 250ms의 지연 시간을 두어 자연스러운 전투 리듬을 확보했습니다.

### [v0.0.24] - 2026-01-18
- **스테이지 점프 연출 디자인 고도화**:
  - **프리미엄 UI 적용**: 캡슐형 반투명 배경과 골드 산세리프 텍스트를 조합한 세련된 팝업 디자인 도입.
  - **애니메이션 시퀀스**: 등장(Scale/Opacity) -> 유지 -> 퇴장(Slide Up/Fade Out)으로 연결되는 부드러운 3단계 애니메이션 구현.
  - **전투 리듬 최적화**: 몬스터 스폰 후 첫 공격까지 250ms의 지연 시간을 두어 자연스러운 전투 리듬을 확보했습니다.

### [v0.0.23] - 2026-01-17
- **절전 및 화면 유지 시스템 도입**: 상단 대시보드 아이콘을 통해 '일반 - 화면 유지 - 절전 모드' 순환 기능을 추가했습니다.
  - **화면 유지**: 모바일 기기 화면이 자동으로 꺼짐을 방지합니다 (`wakelock_plus` 적용).
  - **절전 모드**: 전체 블랙 배경에 무채색 세션 통계(골드, 경험치, 아이템 등)를 표시합니다.
  - **사용성 개선**: 절전 모드 UI를 한글화하고, 해제 방식을 '화면 아무 곳이나 터치'하면 즉시 해제되도록 개선했습니다.
- **강화 마일스톤 기능 일시 비활성화**: 차후 옵션 밸런스 재조정을 위해 +3, +5, +8강 등에서 발생하던 보조 옵션 변화 기능을 중단했습니다.

### [v0.0.21] - 2026-01-17
- **일괄 분해 결과 팝업 구문 오류 수정**: `_showDismantleResultDialog` 함수에서 발생하던 괄호 누락 문제를 해결했습니다.
- **시스템 안정성 및 UI 최적화**: 코드 구조 개선 및 일부 가독성 향상을 위한 리팩토링을 진행했습니다.

### [v0.0.20] - 2026-01-17
- **아이템 옵션 재설정 및 잠금 시스템 구현**:
  - 재설정석을 소모하여 보조 옵션을 리롤하는 기능 추가 (옵션 개수 보존)
  - 개별 옵션 잠금 기능 및 잠금 개수에 따른 마법가루 소모 공식 적용

### [v0.0.19] - 2026-01-17

### [v0.0.18] - 2026-01-17

### [v0.0.17] - 2026-01-17

### [v0.0.16] - 2026-01-17
- **보조 옵션 시스템 전면 개편**: 아이템 티어가 올라갈수록 더 많은 보조 옵션(최대 5개)을 가질 수 있도록 개편했습니다.
- **옵션 랜덤 수치 적용**: 동일 아이템이라도 옵션 수치에 변동폭을 주어 파밍의 재미를 살렸습니다.
- **스탯 연동 시스템 완성**: 그동안 수치로만 존재하던 치명타 확률, 치명타 피해, 공격 속도, 각종 보너스 옵션이 실제 캐릭터 성능에 100% 반영되도록 로직을 통합했습니다.

### [v0.0.15] - 2026-01-17
- **전 티어(T1-T6) 밸런스 일괄 확장**: 전 티어에 걸친 **10배수 성장 모델**을 적용했습니다. 이제 스테이지 100층 단위로 상위 티어를 획득하며 기하급수적인 성장을 경험할 수 있습니다.
- **몬스터 성장 가속도 최적화**: 너무 가파르게 느껴지던 지수 성장을 아이템 티어 도약 주기에 맞춰 $1.025^{stage}$로 안정화했습니다.
- **드랍 시스템 고도화**: 현재 스테이지에 따라 적절한 티어와 등급(Common~Mythic)의 장비가 자동으로 드랍되도록 수정했습니다.

### [v0.0.14] - 2026-01-17
- **전투 밸런스 전면 개편 (지수 모델)**: 몬스터 성장을 지수 함수($1.07^{stage}$)로 변경하여 강화의 효율이 스테이지 돌파로 직결되도록 재설계했습니다.
- **아이템 티어 시스템 확장**: 101 스테이지부터 **T2 아이템**이 드랍됩니다. T2 아이템은 T1 20강에 준하는 기본 성능을 가집니다.
- **데이터 보정 안정화**: 기존 T1 아이템들의 스탯 보정 로직이 새로운 T2 아이템에 영향을 주지 않도록 로직을 정교화했습니다.

### [v0.0.13] - 2026-01-17
- **사망 페널티 강화**: 플레이어 사망 시 현재 스테이지가 5단계 하락하도록 변경하여 패배의 긴장감을 높였습니다.

### [v0.0.12] - 2026-01-17
- **스테이지 가속 시스템 도입**: 내부 난이도는 정교하게 조절하되, 유저에게는 시원시원한 성장감을 주기 위해 표시 스테이지 숫자가 실시간으로 가속되는 시스템을 적용했습니다.
- **전투 밸런스 심화**: 표시 스테이지의 빠른 증가 속도에 맞춰 몬스터의 체력($stage^{1.25}$)과 공격력($stage^{1.1}$) 성장 곡선을 최적화했습니다.

### [v0.0.11] - 2026-01-17
- **초반 구간 전투 난이도 조정 (Smoothing)**: 11스테이지 이전까지 몬스터 체력이 너무 높게 느껴지지 않도록 단계적 완화 로직(1-5층 15%, 6-10층 40%)을 적용했습니다.

### [v0.0.10] - 2026-01-17
- **플레이어 기초 스탯 상향**: 1레벨 플레이어의 기본 공격력(10->30)과 체력(100->250)을 상향 조정했습니다.
- **기존 데이터 소급 적용**: 기존 플레이어도 레벨에 따른 보너스를 포함하여 새로운 기초 능력치 혜택을 받을 수 있도록 보정 로직을 적용했습니다.

### [v0.0.9] - 2026-01-17
- **몬스터 성장 시스템 전면 개편**: 용사의 공격력 상향에 맞춰 몬스터의 기초 체력(900) 및 공격력(90)을 재설정하고, 지수 성장 공식($stage^{1.15}$ 등)을 적용했습니다.
- **전투 밸런스 최적화**: 스테이지 진행에 따른 난이도 체감이 더욱 직관적으로 느껴지도록 수식을 단순화했습니다.

### [v0.0.8] - 2026-01-17
- **기초 밸런스 전면 재조정**: T1 아이템의 기본 스탯을 사용자 요청 수치로 상향 조정했습니다. (공격력 100, 총 HP 1000 등)
- **데이터 자동 보정**: 기존에 보유한 모든 아이템이 새로운 스탯 규격으로 즉시 업데이트되도록 보정 로직을 강화했습니다.

### [v0.0.7] - 2026-01-17
- **장신구 시스템 고도화**: 반지와 목걸이에 **듀얼 주 능력치(공격력+체력)** 시스템을 도입했습니다.
- **강화 로직 통합**: 이제 장신구의 체력 옵션도 공격력과 동일하게 강화 단계에 따라 수치가 상승합니다.
- **UI 최적화**: 아이템 상세창의 주 능력치 영역을 확장하여 두 가지 핵심 스탯을 콤팩트하게 표시하도록 개선했습니다.

### [v0.0.6] - 2026-01-17
- **아이템 시스템 전면 리빌딩 (A안 적용)**: Tier와 Grade 개념을 하나로 통합했습니다.
- **드랍 밸런스 조정**: 현재 모든 사냥터에서 오직 **T1(일반)** 등급의 아이템만 드랍되도록 직관적으로 변경했습니다.
- **코드 무결성**: 모델의 생성자와 직렬화 로직에서 티어와 등급이 항상 동기화되도록 강제했습니다.

### [v0.0.5] - 2026-01-17
- **UI 무결성 개선**: 아이템 슬롯 좌측 상단 티어 라벨이 실제 아이템 티어(`T1`)와 일치하도록 수정 (등급 기반 오표기 해결)

### [v0.0.4] - 2026-01-17
- **개발 환경 최적화**: Windows PowerShell 환경에서의 명령어 호환성 가이드 추가
- **코드 무결성 개선**: UI와 모델 간의 능력치 명칭(HP/공격력) 로직을 `Item` 모델로 통합하여 데이터 불일치 버그 원천 차단

### [v0.0.3] - 2026-01-17
- **장비 시스템 리빌딩**: 1티어 장비 스탯 고정 및 강화 공식(선형 증폭) 적용
- **UI 최적화**: 장착 장비 슬롯 1줄 콤팩트 레이아웃 적용
- **밸런스 조정**: 플레이어 초기 스탯(10/100) 및 드랍률(20%) 조정

### [v0.0.2] - 2026-01-17
- **데이터 영속성**: SharedPreferences 기반 자동 저장/불러오기 구현
- **인벤토리 개선**: 신규 아이템 'N' 표시 및 상세창 네비게이션 추가
- **서버리스 OTA**: GitHub Actions를 통한 자동 APK 빌드 및 업데이트 시스템 구축

### [v0.0.1] - 새로운 시작 (2026-01-17)
- **데이터 보존 시스템**: 이제 앱을 재시작해도 플레이어의 정보가 보존됩니다.
- **관리자 도구 확장**: 실시간 몬스터 방어력 배율(0~100%) 조절 슬라이더가 추가되었습니다.

---

**최종 수정**: 2026-01-17  
**작성자**: Idle Warrior Dev Team
