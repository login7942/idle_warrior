...
> **현재 버전**: v2.3.8 (2026-02-02)  
> **목표**: 서비스 중인 게임의 모든 핵심 시스템, 밸런스 공식, 개발 가이드, 그리고 미래 로드맵을 하나의 문서로 관리하여 데이터 정합성을 유지합니다.
...
| **Map** | **사냥터** | 지역 이동 및 특수 던전(무한의 탑), 사냥터별 드랍 정보 확인 |
| **Inventory** | **가방** | 장비 관리, 분해, 강화, 옵션 재설정 |
| **Craft** | **제작** | 연성 파편과 심연의 가루를 사용한 소모품 제작 |
...
*   **최소 데미지 보장**: 공격력의 **15%**는 방어력과 관계없이 항상 적용. (v0.8.38 상향, 방어 무적 방지)

### 3.3. 소프트캡 시스템 (Soft Cap Rules)
성장 인플레이션을 억제하고 다양한 스탯 투자를 유도하기 위해 특정 구간부터 효율이 감소합니다.

1.  **공격 속도 (AS)**: 5.0 초과 시 `5.0 + (초과분 * 0.4)` 적용. (최대 10.0)
2.  **쿨타임 감소 (CDR)**: 50% 초과 시 `50 + (초과분 * 0.5)` 적용.
3.  **몬스터 보상 (Monster Stage)**: 300층 초과 시 보상 증가율이 로그(log) 스케일로 전환되어 완만하게 상승.
...
*   **공격력 가속 (Atk Scaling)**: 스테이지(S)에 따라 공격력에 추가 가중치 부여
    *   `AtkScaling = 1 + (S / 200)` (v0.8.38 가속, 기존 대비 2.5배 가파른 위협)
*   **베이스 공격력 수치 (v0.8.38 상향)**:
    *   일반 몬스터: 15~24 (기존 대비 50% 상향)
    *   보스 몬스터: 70 (기존 45 대비 상향)
*   **보상 가속 및 소프트캡**: 
    *   **300층 이하**: `최종 보상 = 베이스 * (1 + S / 500)` [지수적 성장]
    *   **300층 초과**: **[Soft Cap]** 보상 베이스에 로그 함수 적용 (성장률 대폭 완화).
...
#### 5.2. 사냥터별 아이템 드랍 티어 (Item Drop Tiers)
| 사냥터 명칭 | 드랍 티어 범위 | 비고 |
| :--- | :---: | :--- |
| **초원 (Grassland)** | T1 | 입문 구간 |
| **숲 (Forest)** | T1 ~ T2 | T2 확률 1.5% |
| **광산 (Mine)** | T1 ~ T3 | T3 확률 1.0% |
| **던전 (Dungeon)** | T1 ~ T4 | T4 확률 0.5% |
| **화산 (Volcano)** | T1 ~ T5 | T5 확률 0.3% |
| **설원 (Snowfield)** | T1 ~ T6 | T6 확률 0.1% |
| **심연 (Abyss)** | T1 ~ T6 | T6 확률 1.0% |

#### 5.3. 강화 시스템 (Enhancement)
*   **슬롯 강화 (v0.3.0)**: 장착 슬롯 자체를 강화(0~3000 레벨). 장비 교체 시에도 수치 유지.
    *   **강화석 소모**: `1 + (현재 레벨 / 50)` 개 소모 (50레벨마다 1개씩 증가)
...
*   **승급(Promotion)**: +20강 장비를 재료와 소모하여 다음 티어 +10강으로 승급.
    *   **승급 비용**: 큐브 대신 강화석+골드 소모로 변경됨.
    *   **골드 비용**: `현재 티어 * 500,000` (현행 대비 10배 상향)
    *   **강화석 비용**:
        *   T1 → T2: 100개
        *   T2 → T3: 1,000개
        *   T3 → T4: 10,000개
...
### 5.3. 아이템 보조 옵션 (Sub-Options) [v2.0 기준]

아이템 등급에 따라 최소 1개에서 최대 6개까지 부여되는 보조 스탯 리스트입니다.

#### 1. 기초 스탯 (Static / 상시 적용)
| 옵션 명칭 | 설명 |
| :--- | :--- |
| **공격력** | 고정 공격력 수치 증가 |
| **공격력(%)** | 공격력 백분율(%) 증가 |
| **체력 / 체력(%)** | 고정 체력 또는 백분율(%) 증가 |
| **방어력 / 방어력(%)** | 고정 방어력 또는 백분율(%) 증가 |
| **공격 속도** | 공격 속도 고정치 증가 (AS 0.01 단위) |
| **치명타 확률/피해** | 치명타 발생 확률 및 피해량 증가 (%) |
| **HP 재생** | 틱당(3초) HP 자동 회복량 증가 |
| **골드/EXP/드롭** | 재화 획득량 및 아이템 드롭 확률 증가 (%) |
| **모든 스킬 레벨** | 현재 장착 및 보유한 모든 스킬 레벨 증가 |
| **최종 피해량 증폭** | 모든 계산 후 최종적으로 가하는 데미지 증폭 (%) |
| **쿨타임 감소** | 전체 액티브 스킬의 쿨타임 감소 (%) |

#### 2. 특수 및 전투 효과 (Special)
| 옵션 명칭 | 설명 |
| :--- | :--- |
| **스킬 추가 발동** | 스킬 시전 시 일정 확률로 동일 스킬 1회 더 발동 |
| **공격 시 보호막 생성** | 공격(일반/스킬) 시 일정 확률로 최대 체력의 5% 보호막 생성 |
| **흡혈** | 가한 데미지의 일정 비율만큼 HP 즉시 회복 |
| **2연타 확률** | 일반 공격 시 일정 확률로 연속 2회 타격 |

#### 3. 회복 및 생존 (v2.0 신규)
| 옵션 명칭 | 설명 |
| :--- | :--- |
| **회복 상한치** | HP 재생(Regen)으로 틱당 회복 가능한 최대치 한계 상향 |
| **피격 시 회복** | 데미지를 입었을 때 받은 피해의 일정 %를 즉시 회복 |
| **스킬 사용시 피해감소 확률** | 스킬 사용 후 5초간 받는 모든 피해 감소 |
| **특정 스킬 쿨감** | 특정 슬롯 스킬의 쿨타임을 %로 추가 감소 |
| **치명타 시 쿨감** | 치명타 발생 시 50% 확률로 쿨타임 0.1~1.0초 즉시 감소 |
| **처형 확률** | 치명타 시 적 HP가 20% 이하일 경우 즉사 확률 |
| **처치 시 공/방증** | 적 처치 시 30초간 공격력 또는 방어력 비폭 증가 (중첩 불가) |
| **지역 이동 시 공/방증** | 사냥터 이동(Zone Move) 시 30초간 공격력/방어력 증가 |
...
| **사냥터 입장 제한** | (지역별 상이) | **숲(360강)**, **광산(900강)**, **던전(1800강)** 등 하한치 존재 |
...
*   **소환 확률 (v0.8.38 하드코어 조정)**:
    *   일반 80% / 고급 15% / 희귀 3% 
    *   영웅 1.8% / 전설 0.18% / **신화 0.02%** (상위 등급 희귀성 대폭 강화)

---

## ⚒️ 8. 제작 및 숙련도 시스템 (Crafting & Mastery)

### 8.1. 제작 숙련도 (Crafting Mastery)
*   **레벨**: 1 ~ 100 레벨
*   **경험치 획득**: 제작 티어당 `티어 * 10` 경험치 획득.
*   **레벨업 보너스**: 레벨당 모든 장비 공격력 +0.5% (최대 +50%)
*   **명장 효과 (Lv.100)**: 제작 시 희귀 등급 이상 등장 확률 +10% 보정.

### 8.2. 세트 아이템 (Set Items)
제작을 통해서만 획득 가능한 특수 장비군입니다.

| 세트 이름 | 티어 | 2세트 효과 | 4세트 효과 |
| :--- | :---: | :--- | :--- |
| **사막의 개척자** | T2 | 골드/EXP +20% | 사냥터 이동 시 30초간 ATK +30% |
| **광산의 수호자** | T3 | 방어력 +20% | 피격 시 10% 확률로 HP 5% 회복 |
| **차원 여행자** | T4 | 스킬 데미지 +25% | 스킬 쿨타임 -15% |
| **드래곤 슬레이어** | T5 | 공격력 +30% | 최종 피해량 증폭 +50% |
| **태고의 신** | T6 | 모든 능력치 +20% | 공격 시 5% 확률로 광역 번개 (공격력 500%) |

---

## 💤 9. 방치 및 효율 시스템 (Idle System)
...
## 🏟️ 10. 무투회 시스템 (Tournament / Arena)

가상의 전사들과 대결하여 오늘의 최강자를 가리는 일일 이벤트 시스템입니다.

### 10.1. NPC 생성 및 스케일링 (v0.8.24)
*   **스냅샷 기반 생성**: NPC 참여 시점의 플레이어 최종 스탯(ATK, HP, DEF 등)을 기준으로 생성됩니다.
*   **라운드별 난이도 가중치**:
    *   **16강**: 배율 1.0x (플레이어와 대등)
    *   **8강**: 배율 1.2x
    *   **4강**: 배율 1.5x
    *   **결승전**: 배율 **2.2x** (최종 챔피언 보정)
*   **챔피언(Champion, 결승전 상대)**:
    *   기본 스탯 위 스케일링 외에 **최대 HP +50%**, **실드 확률 +15%**의 추가 보너스를 받습니다.

### 10.2. NPC 성향 및 특수 능력
*   **성향(Archetype)**: 공격형, 방어형, 기능형, 밸런스형으로 구분되며 자가 버프를 가집니다.
*   **특수 옵션 (NPC 전용)**:
    *   **💀 처형 (Execute)**: 플레이어 HP 20% 이하 시 일정 확률로 즉사 공격.
    *   **🩸 흡혈 (Lifesteal)**: 가한 데미지의 일부를 즉시 체력으로 회복.
    *   **⚡ 반사 (Reflect)**: 받은 데미지의 일부를 플레이어에게 돌려줌.

---

## ♻️ 11. 분해 가이드 (Dismantle Guide)


### 10.1. 일괄/자동 분해 필터링 (Dual Filter)
...
*   **버전**: v2.3.8 (Soft Cap System Introduction)
*   **마지막 업데이트**: 2026-02-02
