...
- **하드캡 (Hard Cap)**:
  - 전투 엔진의 안정성을 위해 행동 간격은 **최소 167ms (초당 6회)**로 제한됩니다.
  - 플레이어 최종 공속은 최대 **6.0**으로 제한됩니다.
- **성장 설계 (최종 6.0 타겟)**:
  - **기본값**: 1.0
  - **스킬/펫**: 약 +3.5 (광폭화 100lv 시 +1.5, 고등급 펫 동행 시 약 +2.0)
  - **아이템**: 6부위 합계 약 +1.5 (T6 극옵 기준 부위당 약 0.2~0.3)
...
- **공격 직렬 처리 (Sequential Logic, v0.4.6)**: 
  캐릭터는 연타 스킬 사용 시 모든 타격(데미지)이 끝날 때까지 다음 공격을 위한 준비(게이지 축적)를 일시 중지합니다. 즉, 하나의 공격 행위가 완결된 후 다음 공격 연계가 실시간으로 이루어지도록 설계되어 있습니다.
...
#### 6.2. 티어 및 품질 기반 수치 설계 (v0.4.0 개편)
*   **성장 모델**: 티어당 **4.0배** 지수 성장 (베이스: T1 무기 ATK 12 등).
*   **강화 효율 (Enhance)**: 공식 대신 **고정 테이블** 참조 방식 적용 (0강 100% ~ 20강 371%).
*   **등급 보너스 (Grade)**: 아이템 등급에 따라 최종 주능력치 추가 배율 적용.
    *   일반(1.0) / 고급(1.1) / 희귀(1.25) / 영웅(1.45) / 고유(1.55) / 전설(1.7) / 신화(2.0)
*   **계산 공식 (Stat Calculation)**:
    1.  **아이템 주능력치**: `ItemStat = Base * TierMult * EnhanceFactor * GradeBonus`
    2.  **캐릭터 적용 수치**: `TotalAppliedValue = ItemStat * SlotEnhanceMultiplier` (슬롯 강화는 장비를 교체해도 유지됨)

#### 6.3. 성장 게이트 (Gate System) - v0.4.8 개편
특정 스펙 달성 시 핵심 기능이 해금되며, 상위 티어 재료가 드랍되기 시작합니다.

**1. 시스템 기능 해금 조건 (슬롯 강화 총합 기준)**
- **아이템 강화**: Lv. **50** 이상
- **옵션 재설정**: Lv. **300** 이상
- **잠재능력 각성**: Lv. **1,000** 이상

**2. 핵심 재료 드랍 및 활성화 조건**
| 목표/재료 | 핵심 재료 (Monster Drop) | 드랍 활성화 조건 (슬롯 강화 총합) | 드랍 가능 지역 |
| :--- | :--- | :--- | :--- |
| **티어 2** | 심연의 구슬 [T2] | Lv. **300** 이상 | 숲, 광산 |
| **티어 3** | 심연의 구슬 [T3] | Lv. **1,000** 이상 | 광산, 던전 |
| **재설정** | **옵션 재설정석** | Lv. **300** 이상 | **숲** 이상 모든 지역 |
| **각성** | **잠재의 큐브** | Lv. **1,000** 이상 | **광산** 이상 모든 지역 |

#### 6.4. 아이템 강화 및 내구도 (v0.4.4 개편)
강화 단계에 따라 성공 확률이 점차 감소하며, 실패 시 내구도가 차등 감소합니다.

**1. 강화 성공 확률**
- **+1 ~ +5**: 100% (안전)
- **+6**: 95% / **+7**: 90% / **+8**: 85% / **+9**: 80% / **+10**: 75%
- **+11**: 65% / **+12**: 60% / **+13**: 55% / **+14**: 50%
- **+15**: 45% / **+16**: 40% / **+17**: 35%
- **+18**: 30% / **+19**: 25% / **+20**: 20%
*※ 강화 단계 하락 페널티는 존재하지 않습니다.*
...
- **15 ~ 17강**: -12 감소
- **18 ~ 20강**: -15 감소
- **파손 상태**: 내구도 0 도달 시 성능 20% 감소 및 추가 강화 불가

**3. 연속 실패 보호 시스템 (Fail Streak Protection)**
연속 강화 실패 시 유저의 부담을 덜어주기 위한 안전장치가 작동합니다.
- **🔰 1단계 보호 (3회 연속 실패)**: 다음 실패 시 내구도 감소량 **50% 감면**
- **🛡️ 2단계 보호 (6회 연속 실패)**: 다음 실패 시 내구도 **감소 없음 (0)** 및 내구도 **1 미만 하락 방지**
- **제한 및 리셋**: 강화 성공 시 또는 2단계 보호 발동 시 실패 카운트는 즉시 0으로 초기화됩니다.

#### 6.5. 제작 시스템 (Crafting System) - v0.5.4 개편
이제 모든 티어의 장비 제작에 **'통합 연성 파편'**이 사용됩니다. 각 티어별 제작 조건은 다음과 같습니다.

| 제작 티어 | 해금 조건 (강화합) | 필요 파편 (Shard) | 필요 구슬 (Core) |
| :--- | :--- | :--- | :--- |
| **Tier 2** | 300 | 300 | T2 구슬 5개 |
| **Tier 3** | 1,000 | 1,000 | T3 구슬 10개 |
| **Tier 4** | 3,000 | 3,000 | T4 구슬 30개 |
| **Tier 5** | 7,500 | 7,500 | T5 구슬 30개 |
| **Tier 6** | 15,000 | 15,000 | T6 구슬 30개 |

#### 6.6. 분해 보상 (Dismantle Rewards)
아이템 분해 시 등급에 따라 **통합 연성 파편**을 획득합니다.
*   **일반**: 1개 / **고급**: 3개 / **희귀**: 10개 / **영웅**: 30개 / **고유**: 60개 / **전설**: 150개 / **신화**: 500개
...
### [2026-01-24] 스킬 시스템 확장 (v0.5.1)
- **신규 스킬 추가**: 「쌍룡참」(act_1_5) - 25레벨 해금, 150% x 2타 (총 300%).
- **기존 스킬 조정**:
  - `바람 베기`: 타격당 80% → 70% (총 210%)로 하향 (초반 밸런스 정상화).
  - `강격`: 단일 대미지 200% → 250%로 상향 (한방 위력 강화).
