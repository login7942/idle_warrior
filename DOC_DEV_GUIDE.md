...
- [x] 로그 이모지 제거
- [x] **Windows 환경 `grep` 명령어 오류**: PowerShell에서 `grep` 미지원으로 인한 중단 이슈 해결 (AI 도구 활용 권장)
...
### [v0.0.2] - 데이터 안정성 강화 (2026-01-17)
- **저장 시스템 보강**: 모든 주요 상호작용(장착, 강화, 스킬 업그레이드 등) 직후 자동 저장이 실행되도록 수정되었습니다.
- **버그 수정**: 데이터 로드 시 일부 Enum(등급 등) 값이 한글로 저장되어 발생하던 충돌을 해결했습니다.
- **성능 개선**: 대량 분해 시 데이터가 즉시 보존됩니다.
